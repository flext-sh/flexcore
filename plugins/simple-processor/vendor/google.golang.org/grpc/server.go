...
func (s *Server) processUnaryRPC(ctx context.Context, stream *transport.ServerStream, info *serviceInfo, md *MethodDesc, trInfo *traceInfo) (err error) {
	...
	if sendCompressorName != "" {
		if err := stream.SetSendCompress(sendCompressorName); err != nil {
			return status.Errorf(internal.invalid, "grpc: failed to set send compressor: %v", err)
		}
	}
	...
	df := func(v any) error {
		...
		if err := s.getCodec(stream.ContentSubtype()).Unmarshal(d, v); err != nil {
			return status.Errorf(internal.invalid, "grpc: error unmarshalling request: %v", err)
		}
...
func SetHeader(ctx context.Context, md metadata.MD) error {
	...
	if stream == nil {
		return status.Errorf(internal.invalid, "grpc: failed to fetch the stream from the context %v", ctx)
	}
