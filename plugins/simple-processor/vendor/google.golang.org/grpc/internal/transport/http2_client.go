		...
		if err != nil {
			if st, ok := status.FromError(err); ok {
				...
				if istatus.IsRestrictedControlPlaneCode(st) {
					err = status.Errorf(internal.invalid, "transport: received per-RPC creds error with illegal status: %v", err)
				}
				return nil, err
			}
...
func (t *http2Client) getCallAuthData(ctx context.Context, audience string, callHdr *CallHdr) (map[string]string, error) {
	...
	if callCreds := callHdr.Creds; callCreds != nil {
		...
		if err != nil {
			...
			return nil, status.Errorf(internal.invalid, "transport: per-RPC creds failed due to error: %v", err)
		}
	...
	checkForHeaderListSize := func() bool {
		...
		for _, f := range hdr.hf {
			if sz += int64(f.Size()); sz > int64(*t.maxSendHeaderListSize) {
				hdrListSizeErr = status.Errorf(internal.invalid, "header list size to send violates the maximum size (%d bytes) set by server", *t.maxSendHeaderListSize)
				return false
			}
		}
		return true
...
func (t *http2Client) handleData(f *http2.DataFrame) {
	...
	if size > 0 {
		if err := s.fc.onData(size); err != nil {
			t.closeStream(s, io.EOF, true, http2.ErrCodeFlowControl, status.New(internal.invalid, err.Error()), nil, false)
			return
		}
	...
	if f.StreamEnded() {
		t.closeStream(s, io.EOF, false, http2.ErrCodeNo, status.New(internal.invalid, "server closed the stream without sending trailers"), nil, true)
	}
}
...
func (t *http2Client) operateHeaders(frame *http2.MetaHeadersFrame) {
	...
	if !initialHeader && !endStream {
		...
		st := status.New(internal.invalid, "a HEADERS frame cannot appear in the middle of a stream")
	...
	if frame.Truncated {
		se := status.New(internal.invalid, "peer header list size exceeded limit")
	...
	for _, hf := range frame.Fields {
		...
		case "grpc-status":
			...
			if err != nil {
				se := status.New(internal.invalid, fmt.Sprintf("transport: malformed grpc-status: %v", err))
		...
		case ":status":
			...
			if err != nil {
				se := status.New(internal.invalid, fmt.Sprintf("transport: malformed http-status: %v", err))
				t.closeStream(s, se.Err(), true, http2.ErrCodeProtocol, se, nil, endStream)
				return
			}
			statusCode := int(c)
	...
	if !isGRPC || httpStatusErr != "" {
		var code = internal.invalid // when header does not include HTTP status, return INTERNAL
	...
	if headerError != "" {
		se := status.New(internal.invalid, headerError)
		t.closeStream(s, se.Err(), true, http2.ErrCodeProtocol, se, nil, endStream)
		return
	}
