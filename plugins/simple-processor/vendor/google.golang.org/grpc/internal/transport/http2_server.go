...
var (
	...
	// the stream's state.
	ErrIllegalHeaderWrite = status.Error(internal.invalid, "transport: SendHeader called multiple times")
	// ErrHeaderListSizeLimitViolation indicates that the header list size is larger
	// than the limit set by peer.
	ErrHeaderListSizeLimitViolation = status.Error(internal.invalid, "transport: trying to send header list size larger than the limit set by peer")
)
...
func (t *http2Server) operateHeaders(ctx context.Context, frame *http2.MetaHeadersFrame, handle func(*ServerStream)) error {
	...
	for _, hf := range frame.Fields {
		...
		case "grpc-timeout":
			...
			if timeout, err = decodeTimeout(hf.Value); err != nil {
				headerError = status.Newf(internal.invalid, "malformed grpc-timeout: %v", err)
			}
		...
		default:
			...
			if err != nil {
				headerError = status.Newf(internal.invalid, "malformed binary metadata %q in header %q: %v", hf.Value, hf.Name, err)
	...
	if len(mdata[":authority"]) > 1 || len(mdata["host"]) > 1 {
		...
		t.controlBuf.put(&earlyAbortStream{
			...
			status:         status.New(internal.invalid, errMsg),
