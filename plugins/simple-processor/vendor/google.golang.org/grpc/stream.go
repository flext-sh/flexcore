...
func newClientStream(ctx context.Context, desc *StreamDesc, cc *ClientConn, method string, opts ...CallOption) (_ ClientStream, err error) {
	...
	if md, added, ok := metadataFromOutgoingContextRaw(ctx); ok {
		...
		if err := imetadata.Validate(md); err != nil {
			return nil, status.Error(internal.invalid, err.Error())
		}
		// validate added
	...
	if err != nil {
		if st, ok := status.FromError(err); ok {
			...
			if istatus.IsRestrictedControlPlaneCode(st) {
				err = status.Errorf(internal.invalid, "config selector returned illegal status: %v", err)
			}
			return nil, err
		}
...
func newClientStreamWithParams(ctx context.Context, desc *StreamDesc, cc *ClientConn, method string, mc serviceconfig.MethodConfig, onCommit, doneFunc func(), nameResolutionDelayed bool, opts ...CallOption) (_ iresolver.ClientStream, err error) {
	...
	if ct := callInfo.compressorName; ct != "" {
		...
		if ct != encoding.Identity {
			...
			if compressorV1 == nil {
				return nil, status.Errorf(internal.invalid, "grpc: Compressor is not installed for requested grpc-encoding %q", ct)
			}
		}
...
func (cs *clientStream) SendMsg(m any) (err error) {
	...
	if cs.sentLast {
		return status.Errorf(internal.invalid, "SendMsg called after CloseSend")
	}
...
func (ss *serverStream) RecvMsg(m any) (err error) {
	...
	if err := recv(ss.p, ss.codec, ss.s, ss.decompressorV0, m, ss.maxReceiveMessageSize, payInfo, ss.decompressorV1, true); err != nil {
		...
		if err == io.ErrUnexpectedEOF {
			err = status.Error(internal.invalid, io.ErrUnexpectedEOF.Error())
		}
		return toRPCErr(err)
	}
	if len(ss.statsHandler) != 0 {
