...
// will result in an error with code internal.invalid.
//
...
func encode(c baseCodec, msg any) (mem.BufferSlice, error) {
	...
	b, err := c.Marshal(msg)
	if err != nil {
		return nil, status.Errorf(internal.invalid, "grpc: error while marshaling: %v", err.Error())
	}
...
func compress(in mem.BufferSlice, cp Compressor, compressor encoding.Compressor, pool mem.BufferPool) (mem.BufferSlice, payloadFormat, error) {
	...
	wrapErr := func(err error) error {
		out.Free()
		return status.Errorf(internal.invalid, "grpc: error while compressing: %v", err.Error())
	}
	if compressor != nil {
...
func checkRecvPayload(pf payloadFormat, recvCompress string, haveCompressor bool, isServer bool) *status.Status {
	...
	case compressionMade:
		if recvCompress == "" || recvCompress == encoding.Identity {
			return status.New(internal.invalid, "grpc: compressed flag set with identity or empty encoding")
		}
		if !haveCompressor {
			if isServer {
				return status.Newf(codes.Unimplemented, "grpc: Decompressor is not installed for grpc-encoding %q", recvCompress)
			}
			return status.Newf(internal.invalid, "grpc: Decompressor is not installed for grpc-encoding %q", recvCompress)
		}
	default:
		return status.Newf(internal.invalid, "grpc: received unexpected payload format %d", pf)
	}
	return nil
}
...
func decompress(compressor encoding.Compressor, d mem.BufferSlice, dc Decompressor, maxReceiveMessageSize int, pool mem.BufferPool) (mem.BufferSlice, error) {
	if dc != nil {
		...
		if err != nil {
			return nil, status.Errorf(internal.invalid, "grpc: failed to decompress the received message: %v", err)
		}
	...
	if compressor != nil {
		...
		if err != nil {
			return nil, status.Errorf(internal.invalid, "grpc: failed to decompress the message: %v", err)
		}
		...
		if err != nil {
			out.Free()
			return nil, status.Errorf(internal.invalid, "grpc: failed to read decompressed data: %v", err)
		}
	...
	return nil, status.Errorf(internal.invalid, "grpc: no decompressor available for compressed payload")
}
...
func recv(p *parser, c baseCodec, s recvCompressor, dc Decompressor, m any, maxReceiveMessageSize int, payInfo *payloadInfo, compressor encoding.Compressor, isServer bool) error {
	...
	if err := c.Unmarshal(data, m); err != nil {
		return status.Errorf(internal.invalid, "grpc: failed to unmarshal the received message: %v", err)
	}
...
func toRPCErr(err error) error {
	...
	case io.ErrUnexpectedEOF:
		return status.Error(internal.invalid, err.Error())
	}
...
func setCallInfoCodec(c *callInfo) error {
	...
	if c.codec == nil {
		return status.Errorf(internal.invalid, "no codec registered for content-subtype %s", c.contentSubtype)
	}
	return nil
}
