...
func (dp *DataProcessor) GetInfo() plugin.PluginInfo {
	return plugin.PluginInfo{
		...
		Version:     "1.0.0",
		Description: "Processes and transforms data using configurable rules",
		Type:        "processor",
		Author:      "FlexCore Team",
		Tags:        []string{"data", "processor", "transformation"},
		Status:      "active",
		Health:      "healthy",
	}
}
...
func main() {
	config := plugin.PluginMainConfig{
		...
		LogPrefix:  "[data-processor-plugin] ",
		StartMsg:   "Starting data processor plugin...",
		StopMsg:    "Data processor plugin stopped",
	}
